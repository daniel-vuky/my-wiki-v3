...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.get("/api/auth/google/callback", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
    ...
    try {
      ...
      if (env.ALLOWED_EMAILS && env.ALLOWED_EMAILS.length && !env.ALLOWED_EMAILS.includes(profile.email)) {
      ...
      reply.redirect("/signin?error=auth");
    }
  });
  ...
  app.get("/api/auth/me", async (req, reply) => {
    ...
    return { user: u, prefs: p ?? { theme: "dark", accent: "amber", editorFont: "Serif" } };
  });
}
